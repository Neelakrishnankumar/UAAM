...
import { customerorderanalysis, productorderanalysis, resetTrackingData } from "../../../store/reducers/Formapireducer";
...
const ProductAnalysis = () => {
  ...
  const seriesData = useSelector((state) => state.formApi.productanalysisData);
  ...
  const childToParent = async (childdata, type) => {
    // console.log("🚀 ~ file: ProductAlaysis.jsx:41 ~ childToParent ~ childdata:", childdata)
    if (type == "Customer") {
      setisPopupdata(true);
      setCustomerLookup({
...
const productorderFn = async () => {
    if(productLookup.proRecordID){
      setLoading(true)

      const data = { Query: { RecordID: productLookup.proRecordID, CompanyID:compID, Finyear:YearRecorid} }

      const response = await dispatch( productorderanalysis({data}));
       console.log("🚀 ~ file: Editreport.jsx:266 ~ reportFn ~ response:", JSON.stringify(response.payload))
    ...
    <React.Fragment>
      <Box display="flex" justifyContent="space-between" p={2}>
        <Box
          ...
          alignItems="center"
        >
          <Typography variant="h3">Product Order Analysis</Typography>
        </Box>
        <Box display="flex">
...
<IconButton onClick={() =>   fnLogOut('Close')} color="error">
    ...
    <Box
            ...
            <Box display="flex" justifyContent="end" mt="20px" gap="20px">
              <LoadingButton loading={loading} variant="contained" color="secondary" onClick={productorderFn}>
                APPLY
              </LoadingButton>
              <Button onClick={() => {
            ...
            {seriesData && seriesData.categories &&(<Chart
                            ...
                            options={{
                              ...
                              xaxis: {
                                ...
                                labels:{
                                  formatter:  value => {
                                    // console.log("🚀 ~ file: ProductAlaysis.jsx:241 ~ ProductAnalysis ~ value:", value)
                                   return value ?   `${value["IP_DATE"]} | ${value["PRD_MODELNO"]}` : "hell"
                                  }
                                }
